<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Favourites') }} - <a href="{{route('favourites')}}">{{request()->category->title}}</a>
        </h2>
    </x-slot>
                        ...
                        <div class="pt-3 sm:pt-5">
                            <div class="flex justify-between items-center">
                                <h2 class="text-xl font-semibold text-black dark:text-white">
                                    <a href="{{ route('url.details', $favourite->id) }}">
                                        {{ $favourite->title }}
                                    </a>
                                </h2>
                                <a href="#" class="ml-4">
                                    @if(auth()->user()->favourites()->where(['favouritable_id' => $favourite->id, 'favouritable_type' => App\Models\Url::class])->exists())
                                        <form action="{{ route('favourites.destroy', ['type' => 'url', 'id' => $favourite->id]) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @else
                                        <form action="{{ route('favourites.store', ['type' => 'url', 'id' =>  $favourite->id]) }}" method="POST" class="inline">
                                            @csrf
                            ...
                            <div class="mt-4 flex items-center">
                                ...
                                @endfor
                                <a href="{{ route('url.reviews',  $favourite->id) }}" class="ml-2 text-gray-600">({{  $favourite->reviews->count() }} reviewers)</a>
                            </div>
